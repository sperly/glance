...
WindowSettings SettingsManager::LoadWindowSettings() const
{
    ...
    long width = 0;
    long height = 0;
    settings.hasGeometry = config.Read("X", &x) &&
                           config.Read("Y", &y) &&
...
wxArrayString SettingsManager::LoadRecentItems(const wxString& groupName) const
{
    wxConfig config("Glance");
...
void SettingsManager::SaveRecentItems(const wxString& groupName, const wxArrayString& items) const
{
    wxConfig config("Glance");
...
void SettingsManager::AddRecentItem(const wxString& groupName, const wxString& path) const
{
    if (path.empty())
    {
        return;
