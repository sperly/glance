...
wxBEGIN_EVENT_TABLE(MainFrame, wxFrame)
    EVT_MENU(ID_NEW_FILE, MainFrame::OnFileNewFile)
    EVT_MENU(ID_OPEN_FOLDER, MainFrame::OnFileOpenFolder)
    EVT_MENU(wxID_OPEN, MainFrame::OnFileOpenFile)
    EVT_MENU(wxID_SAVE, MainFrame::OnFileSave)
    EVT_MENU(ID_SAVE_ALL, MainFrame::OnFileSaveAll)
    EVT_MENU(ID_CLOSE_TAB, MainFrame::OnFileCloseTab)
    EVT_MENU(wxID_PRINT, MainFrame::OnFilePrint)
    EVT_MENU(wxID_EXIT, MainFrame::OnFileExit)
    EVT_MENU_RANGE(ID_RECENT_FILE_BASE, ID_RECENT_FILE_LAST, MainFrame::OnRecentFile)
    EVT_MENU_RANGE(ID_RECENT_FOLDER_BASE, ID_RECENT_FOLDER_LAST, MainFrame::OnRecentFolder)
    EVT_MENU(ID_CLEAR_RECENT_ITEMS, MainFrame::OnClearRecentItems)
    ...
    EVT_MENU_RANGE(ID_FORMAT_BOLD, ID_FORMAT_CLEAR_FORMATTING, MainFrame::OnFormatCommand)
    EVT_MENU_RANGE(ID_INSERT_LINK, ID_INSERT_TOC, MainFrame::OnInsertCommand)
    EVT_MENU(ID_HELP_SHOW_HELP, MainFrame::OnHelpShowHelp)
    EVT_MENU(ID_HELP_SAVE_PREVIEW_HTML, MainFrame::OnHelpSavePreviewHtml)
      ...
      m_recentFoldersMenu(nullptr)
{
    ...
    wxMemoryInputStream iconStream(GetEmbeddedGlanceIconPngData(), GetEmbeddedGlanceIconPngSize());
...
void MainFrame::CreateMenuBar()
{
    ...
    fileMenu->Append(ID_NEW_FILE, "&New File...\tCtrl+N", "Create a new Markdown file");
    fileMenu->Append(ID_OPEN_FOLDER, "Open &Folder...\tCtrl+Shift+O", "Open a folder containing Markdown files");
    ...
    fileMenu->Append(ID_CLEAR_RECENT_ITEMS, "Clear Recent &Items", "Clear recent files and folders");
    fileMenu->AppendSeparator();
    fileMenu->Append(wxID_SAVE, "&Save File\tCtrl+S", "Save the current file");
    fileMenu->Append(ID_SAVE_ALL, "Save &All\tCtrl+Shift+S", "Save all modified files");
    fileMenu->AppendSeparator();
    fileMenu->Append(ID_CLOSE_TAB, "&Close Tab\tCtrl+W", "Close the current editor tab");
    ...
    formatMenu->Append(ID_FORMAT_BLOCKQUOTE, "Block&quote", "Prefix selected lines as a blockquote");
    ...
    helpMenu->Append(ID_HELP_SHOW_HELP, "&Help\tF1", "Open the Glance help guide");
    helpMenu->Append(ID_HELP_SAVE_PREVIEW_HTML, "Save Preview &HTML...", "Save the rendered preview HTML to a file");
...
void MainFrame::CreateLayout()
{
    ...
    m_fileTreePanel->Bind(wxEVT_GLANCE_FILE_SELECTED, &MainFrame::OnMarkdownFileSelected, this);
    m_fileTreePanel->Bind(wxEVT_GLANCE_FILE_ACTIVATED, &MainFrame::OnMarkdownFileActivated, this);
    ...
    m_editorNotebook->Bind(wxEVT_GLANCE_EDITOR_STATUS_CHANGED, &MainFrame::OnEditorStatusChanged, this);
    m_editorNotebook->Bind(wxEVT_GLANCE_ACTIVE_DOCUMENT_CHANGED, &MainFrame::OnActiveDocumentChanged, this);
    m_editorNotebook->Bind(wxEVT_GLANCE_DOCUMENT_CHANGED, &MainFrame::OnDocumentChanged, this);

    m_previewPanel = new PreviewPanel(m_editorPreviewSplitter);

    m_editorPreviewSplitter->SplitVertically(m_editorNotebook, m_previewPanel, 500);
...
{
    if (m_editorNotebook->HasModifiedDocuments() && !m_editorNotebook->ConfirmCloseAll())
    {
...
bool MainFrame::OpenFile(const wxString& filePath)
{
    ...
    {
        wxMessageBox("Only Markdown files can be opened.", "Unsupported File", wxOK | wxICON_WARNING, this);
        return false;
    }
...
void MainFrame::OnFileNewFile(wxCommandEvent& event)
{
    wxFileDialog dialog(this,
                        "New Markdown File",
                        GetDefaultNewFileDirectory(),
                        "Untitled.md",
                        "Markdown files (*.md;*.markdown;*.mdown;*.mkd)|*.md;*.markdown;*.mdown;*.mkd|All files (*.*)|*.*",
                        wxFD_SAVE);
...
void MainFrame::OnFilePrint(wxCommandEvent& event)
{
    ...
    if (!document)
    {
        wxMessageBox("No document is open.", "Print", wxOK | wxICON_INFORMATION, this);
        return;
    }
...
void MainFrame::OnInsertCommand(wxCommandEvent& event)
{
    ...
    case ID_INSERT_IMAGE:
    {
        wxFileDialog fileDialog(this,
                                ...
                                "",
                                "",
                                "Image files (*.png;*.jpg;*.jpeg;*.gif;*.svg;*.webp)|*.png;*.jpg;*.jpeg;*.gif;*.svg;*.webp|All files (*.*)|*.*",
                                wxFD_OPEN | wxFD_FILE_MUST_EXIST);
        ...
        wxTextEntryDialog altDialog(this, "Alt text:", "Insert Image", imageName.GetName());
...
void MainFrame::OnHelpSavePreviewHtml(wxCommandEvent& event)
{
    ...
    if (!file.IsOpened() || !file.Write(m_previewPanel->GetHtmlSource(), wxConvUTF8))
    {
...
void MainFrame::OnDocumentChanged(wxCommandEvent& event)
{
    UpdatePreview();
}
...
{
    if (event.GetActive() && m_editorNotebook && m_editorNotebook->CheckForExternalChanges(this))
    {
        UpdatePreview();
...
bool MainFrame::CreateNewFile(const wxString& filePath)
{
    ...
    const wxString currentFolder = m_fileTreePanel ? m_fileTreePanel->GetCurrentFolder() : wxString();
...
{
    if (!m_editorNotebook->ExecuteMarkdownCommand(command, argument, secondaryArgument))
    {
...
wxString MainFrame::MakeMarkdownImagePath(const wxString& imagePath) const
{
    ...
    const wxString currentFilePath = m_editorNotebook ? m_editorNotebook->GetCurrentFilePath() : wxString();
...
void MainFrame::ApplyWindowSettings()
{
    ...
    {
        SetSize(settings.position.x,
                settings.position.y,
                settings.size.GetWidth(),
...
void MainFrame::SaveWindowSettings()
{
    ...
    settings.fileTreeSash = m_splitter ? m_splitter->GetSashPosition() : settings.fileTreeSash;
    settings.previewSash = m_editorPreviewSplitter ? m_editorPreviewSplitter->GetSashPosition() : settings.previewSash;
    m_settingsManager.SaveWindowSettings(settings);
}

void MainFrame::RefreshRecentMenus()
{
    auto repopulateMenu = [this](wxMenu* menu,
        ...
        {
            menu->Append(firstId + static_cast<int>(i), FormatRecentMenuLabel(i, items[i]));
        }
    };

    repopulateMenu(m_recentFoldersMenu, m_recentFolders, ID_RECENT_FOLDER_BASE, "(No recent folders)");
    repopulateMenu(m_recentFilesMenu, m_recentFiles, ID_RECENT_FILE_BASE, "(No recent files)");

    if (wxMenuBar* menuBar = GetMenuBar())
    {
        menuBar->Enable(ID_CLEAR_RECENT_ITEMS, !m_recentFiles.IsEmpty() || !m_recentFolders.IsEmpty());
    }
}
...
wxString MainFrame::FormatRecentMenuLabel(size_t index, const wxString& path) const
{
    wxString label = path;
    label.Replace("&", "&&");
