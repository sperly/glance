...
Document* EditorNotebook::OpenFile(const wxString& filePath, wxString* errorMessage)
{
...
bool EditorNotebook::CheckForExternalChanges(wxWindow* parent)
{
    ...
    for (Document* document : documentsToCheck)
    {
        ...
        wxString message = document->GetFileName() + " changed outside Glance.\n\nReload it from disk?";
        if (document->IsModified())
        {
            message += "\n\nReloading will discard your unsaved editor changes.";
        }

        wxMessageDialog dialog(parent ? parent : this,
                               message,
        ...
        if (response == wxID_YES)
        {
            ...
            {
                wxMessageBox(errorMessage, "Reload Failed", wxOK | wxICON_ERROR, parent ? parent : this);
...
bool EditorNotebook::PromptSaveIfModified(Document* document)
{
    ...
    wxMessageDialog dialog(this,
                           "Save changes to " + document->GetFileName() + "?",
                           "Unsaved Changes",
...
bool EditorNotebook::ReloadDocument(Document* document, wxString* errorMessage)
{
    const int page = FindPageForDocument(document);
    if (page == wxNOT_FOUND || !document)
    {
