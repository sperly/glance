...
bool FileTreePanel::OpenFolder(const wxString& folderPath) {
  ...
  if (!wxDir::Exists(normalizedPath)) {
    wxMessageBox("Folder does not exist: " + normalizedPath, "Error",
                 wxOK | wxICON_ERROR);
    return false;
  }
  ...
  m_rootItem = m_treeCtrl->AddRoot(folderName.GetFullName().empty()
                                       ? normalizedPath
                                       : folderName.GetFullName());
...
wxString FileTreePanel::GetCurrentFolder() const { return m_currentFolder; }

bool FileTreePanel::AddFolderToTree(wxTreeItemId parentItem,
...
void FileTreePanel::AddFileToTree(wxTreeItemId parentItem,
...
void FileTreePanel::SendFileEvent(const wxTreeItemId& item,
                                  wxEventType eventType) {
  if (!item.IsOk() || item == m_rootItem) return;

  FileTreeItemData* itemData =
      dynamic_cast<FileTreeItemData*>(m_treeCtrl->GetItemData(item));
  if (!itemData) return;
